...
class EndEffectorAction(Action):
    ...
    def execute_bebs(
        ...
        control_states = []
        env.step_fn_callbacks["visualization_callback"] = (
            config.visualization_fps,
            visualization_callback,
        )
        env.step_fn_callbacks["control_states_callback"] = (
            env.config.ctrl.frequency,
        ...
        env.obs = next_obs
        if "visualization_callback" in env.step_fn_callbacks:
            del env.step_fn_callbacks["visualization_callback"]
        if "control_states_callback" in env.step_fn_callbacks:
            del env.step_fn_callbacks["control_states_callback"]
...
class ControlAction(Action):
    ...
    def get_interpolated_joint_ctrl(
        ...
        if self.config.control_type == ControlType.JOINT:
            ...
            from environment.mujocoRobot import MujocoRobot
...
class GraspLinkPoseAction(LinkPoseAction):
    ...
    def check_feasibility(self, env: Env):
        ...
        from environment.mujocoEnv import MujocoEnv
        from environment.utils import get_part_path
...
class GraspLinkAction(HighLevelLinkAction):
    ...
    # make GraspLinkPoseAction, take link_point_cloud and env as input 
    def get_feasible_candidate(self, link_point_cloud: PointCloud, env: Env) -> Policy:
        obs = env.obs
        state = obs.state
        numpy_random = env.policy_numpy_random
    ...
    def sample(
        ...
        if self.do_reset:
            try:
                ...
                try:
                    obs = self.env.reset(episode_id=(episode_id+1000000))
                except:
                    obs = self.env.reset(episode_id=(episode_id+10000000))
        else:
...
class EnvConfig:
    ...
    num_action_candidate_attempt: int
    num_action_candidates_for_average: int
    # grasp action
    max_pushin_dist: float = 0.05
    min_pushin_dist: float = -0.01
    # pick action
    prepick_distance: float = 0.1
    # revolution action
...
class Env(ABC):
    def __init__(
        ...
        self.record_video = False  # bebs
        self.env_sampler_config = None # bebs
    ...
    def execute_ctrl(
        ...
        while self.time < self.control_buffer.end_time:
            ...
            # bebs: record video
            if self.record_video and self.__step_counter % 40 == 0:
                frame = self.get_rgb(camera_name='front')
                self.video_buffer.append(frame)
    
    # bebs: record video
    def start_record(self, video_fps=20):
        self.record_video = True
        self.video_fps = video_fps
        # self.duration = 1000 / video_fps
        self.video_buffer = []
    def end_record(self, path):
        self.record_video = False
        imageio.mimsave(str(path), self.video_buffer, fps=self.video_fps)
